...
###### Dataset
df_ddi_train = pd.read_csv('drugbank_test/inductive/fold1/train.csv')
df_ddi_s1 = pd.read_csv('drugbank_test/inductive/fold1/s1.csv')
df_ddi_s2 = pd.read_csv('drugbank_test/inductive/fold1/s2.csv')
...
def test(s1_data_loader, s2_data_loader, model):
    ...
    with torch.no_grad():
        for batch in s1_data_loader:
            model.eval()
            p_score, n_score, probas_pred, _,ground_truth = do_compute(batch, device, model=model)
            s1_probas_pred.append(probas_pred)
            s1_ground_truth.append(ground_truth)
